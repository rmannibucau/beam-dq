...
public abstract class NumberColumnAnalyser extends ColumnAnalyser<Number, Double, Double> {
    ...
    @Override
    public Double mergeAccumulators(final Iterable<Double> accumulators) {
        return merge(stream(accumulators.spliterator(), false).mapToDouble(i -> i)).orElse(Double.NaN);
    }

    protected abstract OptionalDouble merge(final DoubleStream stream);
}
