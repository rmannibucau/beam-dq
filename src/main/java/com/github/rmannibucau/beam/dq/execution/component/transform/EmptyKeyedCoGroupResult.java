...
public class EmptyKeyedCoGroupResult<A> extends DoFn<A, KV<Integer, CoGbkResult>> {
    @ProcessElement
    public void onElement(final OutputReceiver<KV<Integer, CoGbkResult>> output) {
        output.output(KV.of(1, CoGbkResult.empty()));
    }
    ...
    private static class Tfn<A> extends PTransform<PCollection<A>, PCollection<KV<Integer, CoGbkResult>>> {
        private final EmptyKeyedCoGroupResult<A> fn;

        @Override
        public PCollection<KV<Integer, CoGbkResult>> expand(final PCollection<A> input) {
            return input.apply("EmptyKeyedCoGroupResult", ParDo.of(fn));
        }
    }
}
